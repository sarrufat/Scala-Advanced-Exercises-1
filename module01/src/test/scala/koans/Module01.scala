...
class Module01 extends KoanSuite with Matchers {
  ...
  class CalorieTracker {
    ...
    def dailyMax_=(dm: Int): Unit = {
      require(dm > 0 && dm < 5000)
      _dailyMax = dm
    }

    var _currentDaily: Int = 0

    def currentDaily: Int = _currentDaily

    def currentDaily_=(cd: Int): Unit = {
      require(cd > -500 && cd < (_dailyMax + 500))
      _currentDaily = cd
    }
  ...
  object DBCache {
    ...
    val recipeCache = CacheBuilder.newBuilder().build {
      // implementation of cache using cachebuilder goes here.
      new CacheLoader[String, Future[List[String]]] {
        def load(key: String): Future[List[String]] = Future({
