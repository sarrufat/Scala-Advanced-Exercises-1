...
class Person(val name: String) extends HeightAndWeight {
	...
	override def height: Double = ht
	override def height_=(h: Double): Unit = {
		require(h > 0.0, "Height may not be zero or negative")
		ht = h
	}

	override def weight: Double = wt
	override def weight_=(w: Double): Unit = {
...
class TruckLoad extends HeightAndWeight {
	...
	override def height: Double = propsMap.getOrElse("height", 0.0)
	override def height_=(h: Double): Unit = propsMap("height") = h

	override def weight: Double = propsMap.getOrElse("weight", 0.0)
	override def weight_=(w: Double): Unit = propsMap("weight") = w
